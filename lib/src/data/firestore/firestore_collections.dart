...
class FirestoreCollections {
  ...
  static Map<String, Object?> _userToFirestore(AppUser user) {
    return {
      ...
      'email': user.email,
      // The two profile-image fields (photoPath, avatarId) are NEVER written
      // from here. This map goes out as a whole-document merge from
      // switchMode and updateProfile, carrying an in-memory copy that is
      // routinely stale: writing a null would erase an image set on another
      // device, and writing a value would bring back one erased there, next
      // to its replacement. UserRepository.setProfileImage owns both fields
      // and writes them together, with FieldValue.delete() for a null.
      ...
      // predates a reload. Unlike the image fields it is safe to write when
      // set: gender is declared once at sign-up and has no other writer.
