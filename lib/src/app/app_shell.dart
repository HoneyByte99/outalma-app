...
class _AppShellState extends ConsumerState<AppShell>
  ...
  Widget build(BuildContext context) {
    ...
    void onTap(int logicalIndex) {
      ...
      // A visitor with no account owns tab 0 only: the other three list
      // bookings, chats and their own profile, none of which exist yet. The
      // tabs stay VISIBLE rather than being removed, because hiding them would
      // hide what an account is for; tapping one explains and offers to create
      // it, and returns to that same tab afterwards.
      if (isGuest && logicalIndex != 0) {
        showAuthPrompt(
          context,
          reason: l10n.guestLockedSectionLogin,
          redirect: _guestTabTarget(logicalIndex),
        );
        return;
      }
...
/// Where a guest lands after signing in from a locked tab.
///
/// Only the CLIENT tab layout is mapped: activeModeProvider resolves to client
/// for anyone who is not authenticated, so a guest never sees the provider bar.
String _guestTabTarget(int logicalIndex) {
  switch (logicalIndex) {
    case 1:
