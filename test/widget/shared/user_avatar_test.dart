  ...
  group('UserAvatar', () {
    testWidgets('empty displayName shows Icons.person_rounded', (tester) async {
      await tester.pumpWidget(_wrap(
        const UserAvatar(displayName: '', photoPath: null),
      ));
      await tester.pump();
    ...
    testWidgets('single-word displayName shows first letter initial',
        (tester) async {
      await tester.pumpWidget(_wrap(
        const UserAvatar(displayName: 'Alice', photoPath: null),
      ));
      await tester.pump();
    ...
    testWidgets('two-word displayName shows first + last initials',
        (tester) async {
      await tester.pumpWidget(_wrap(
        const UserAvatar(displayName: 'Alice Dupont', photoPath: null),
      ));
      await tester.pump();
    ...
    testWidgets('photoPath null shows initials widget, no CachedNetworkImage',
        (tester) async {
      await tester.pumpWidget(_wrap(
        const UserAvatar(displayName: 'Bob Martin', photoPath: null),
      ));
      await tester.pump();
    ...
    testWidgets('radius parameter changes container size', (tester) async {
      ...
      await tester.pumpWidget(_wrap(
        const UserAvatar(
            displayName: 'Test User', photoPath: null, radius: radius),
      ));
      await tester.pump();
      ...
      final avatarFinder = find.byWidgetPredicate((w) =>
          w is Container &&
          w.decoration is BoxDecoration &&
          (w.decoration as BoxDecoration).shape == BoxShape.circle);
      expect(avatarFinder, findsOneWidget);
      final renderBox =
          tester.renderObject<RenderBox>(avatarFinder.first);
      expect(renderBox.size.width, equals(radius * 2));
      expect(renderBox.size.height, equals(radius * 2));
    });

    testWidgets('whitespace-only displayName shows person icon', (tester) async {
      await tester.pumpWidget(_wrap(
        const UserAvatar(displayName: '   ', photoPath: null),
      ));
      await tester.pump();
